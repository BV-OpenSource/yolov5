...
from utils.dataloaders import IMG_FORMATS, VID_FORMATS, LoadImages, LoadStreams, urlparse
...
import rospy
import std_msgs
from bev_uav_msgs.msg import DetectionBoundingBox
...
running = False # should it be running
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                bbox_msg.header.seq = seen

#                bbox_msg.header.timestamp&/
                # Print results
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    bbox = BoundingBox2D()
                    objScore = ObjectHypothesis()

                    bbox.size_x = (xyxy[2]-xyxy[0])/im0.shape[1]
                    bbox.size_y = (xyxy[3]-xyxy[1])/im0.shape[0]
                    bbox.center.x = (xyxy[2]+xyxy[0])/(2 * im0.shape[1])
                    bbox.center.y = (xyxy[3]+xyxy[1])/(2 * im0.shape[0])
                    bbox_msg.bboxes.append(bbox)

                    objScore.id = int(cls)
                    objScore.score = float(conf)
                    bbox_msg.results.append(objScore)

                if running != False:
                    bboxPub.publish(bbox_msg)
                bbox_msg.bboxes.clear()
                bbox_msg.results.clear()
...
def main(opt):
    ...
    rospy.init_node('yolov5_node', anonymous=True, log_level=rospy.INFO) # rospy.loginfo() calls must be done after this
    ros_stream_handler = RosStreamHandler()
    ros_stream_handler.setLevel(rospy.DEBUG)
